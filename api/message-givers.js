...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'sms') {
      ...
      for (const giver of givers.filter(g => g.phone)) {
        try {
          const clean = giver.phone.replace(/\D/g, '');
          const to = clean.startsWith('1') ? '+' + clean : '+1' + clean;
          await twilioClient.messages.create({ body, from: process.env.TWILIO_PHONE_NUMBER, to });
          sent++;
        } catch (e) { console.warn('SMS failed for', giver.phone, e.message); }
      }
    } else {
      const resend = new Resend(process.env.RESEND_API_KEY);
      for (const giver of givers.filter(g => g.email)) {
        try {
          await resend.emails.send({
            ...
            html: `<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;padding:40px 24px;color:#1A1613;">
              ...
              <div style="font-size:16px;line-height:1.8;color:#6B6560;">${body.replace(/\n/g, '<br>')}</div>
              <p style="font-size:13px;color:#6B6560;margin-top:32px;">— Apostle Joshua & Kingdom Insights Ministries<br>kim.church</p>
            </div>`
          });
          sent++;
        } catch (e) { console.warn('Email failed for', giver.email, e.message); }
      }
    }
