...
module.exports = async function handler(req, res) {
  ...
  // ── OFFLINE GIVING INTENT ─────────────────────────────────────────────────
  if (action === 'offline-intent') {
    ...
    try {
      await resend.emails.send({
        ...
        html: `<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;color:#1A1613;background:#ffffff;">
          <div style="background:#ffffff;padding:24px 24px 16px;text-align:center;border-bottom:3px solid #C8922A;">
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;color:#1A1613;background-color:#ffffff;">
        <div style="background:#ffffff;padding:24px 24px 16px;text-align:center;border-bottom:3px solid #C8922A;margin-bottom:32px;">
          <img src="https://kim.church/kim-logo-email.png" alt="Kingdom Insights Ministries" style="height:60px;">
        </div>
        <div style="padding:0 24px 24px;">
          ...
          <h1 style="font-size:32px;font-weight:600;line-height:1.2;margin-bottom:16px;">Thank you, ${firstName}.</h1>
          <p style="font-size:16px;line-height:1.8;color:#6B6560;">Your gift of <strong style="color:#1A1613;">$${amountFmt}</strong> (${freqLabel}) has been received. You are writing your name into the foundation of something God is building.</p>
          <div style="margin:32px 0;padding:24px;background:#FBF4E8;border-left:3px solid #C8922A;">
            <p style="font-size:14px;font-style:italic;color:#6B6560;line-height:1.7;margin:0;">"Each of you should give what you have decided in your heart to give — not reluctantly or under compulsion, for God loves a cheerful giver." — 2 Corinthians 9:7</p>
          </div>
          <p style="font-size:15px;line-height:1.8;color:#6B6560;">Apostle Joshua and the KIM team will be praying over every partner by name.</p>
    ...
    await resend.emails.send({
      ...
      html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
        <h2 style="color:#C8922A;">New Gift Received</h2>
        <table style="width:100%;font-size:14px;">
          <tr><td style="padding:8px 0;color:#666;">Name</td><td style="padding:8px 0;font-weight:600;">${full_name}</td></tr>
          <tr><td style="padding:8px 0;color:#666;">Email</td><td>${email}</td></tr>
          <tr><td style="padding:8px 0;color:#666;">Amount</td><td style="font-weight:600;">$${amountFmt} (${freqLabel})</td></tr>
          <tr><td style="padding:8px 0;color:#666;">Tier</td><td>${tier || 'Not selected'}</td></tr>
        </table>
      </div>`
    });
